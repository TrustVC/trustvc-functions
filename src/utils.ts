...
): OpenAttestationDocument | SignedVerifiableCredential | unknown => {
...
export const validateNetwork = (
  ...
  if (vc.isSignedDocument(document) || vc.isRawDocument(document)) {
    if (isTransferableRecord(document)) {
      ...
      if (!tokenNetwork) {
        throw createError(400, ERROR_MESSAGE.DOCUMENT_NETWORK_NOT_FOUND);
      }
      return tokenNetwork;
    } else {
      return {};
    ...
    throw createError(400, ERROR_MESSAGE.DOCUMENT_SCHEMA_INVALID);
  }
};
...
export const validateDocument = async ({
  ...
  if (network) {
    ...
    if (!supportedNetwork) {
      throw createError(400, ERROR_MESSAGE.NETWORK_UNSUPPORTED);
    }
  ...
  if (!isValid(fragments)) {
    throw createError(400, ERROR_MESSAGE.DOCUMENT_GENERIC_ERROR);
  }
  return fragments;
};
